...
// PomodoroFocusView: helper methods kept here — its body reads private member(s): isPickerPresented, screenTime.
//   To move this into Patch/Generated/, make those member(s) `internal` (drop
//   `private`/`fileprivate`) and re-run `patchcli prepare`.
// PomodoroMainTimerCard: helper methods kept here — its body reads private member(s): sessionModeText.
//   To move this into Patch/Generated/, make those member(s) `internal` (drop
//   `private`/`fileprivate`) and re-run `patchcli prepare`.
...
extension PomodoroFocusView {
    ...
    @MainActor func __patchSlots() -> [String: ([String]) -> AnyView] {
        ...
        __s["op_6767b8de6f4a17be"] = { (_: [String]) in AnyView(ScrollView(showsIndicators: false) {
            VStack(spacing: HIGSpacing.md) {
                // 1. Kartu Timer Utama dengan Progress Ring & Pulse
                PomodoroMainTimerCard(pomodoro: pomodoro)
...
extension PomodoroMainTimerCard {
    /// Native renderers for this view's non-lowerable leaves, keyed by the
    /// shipped tree's opaque-slot id. Each is a FACTORY `([String]) -> AnyView`:
    /// a PARAMETERIZED leaf (a slotted custom view with lifted string-literal
    /// args) substitutes the runtime-supplied `a[k]` into its template, so an
    ...
    @MainActor func __patchSlots() -> [String: ([String]) -> AnyView] {
        var __s: [String: ([String]) -> AnyView] = [:]
        __s["op_6f4fe9c3b85e69a8"] = { (a: [String]) in a.count >= 4 ? AnyView(VStack(spacing: HIGSpacing.md) {
            // Header Status Sesi
            HStack {
                HStack(spacing: 6) {
                    Circle()
            ...
            ZStack {
                ...
                VStack(spacing: 2) {
                    ...
                    if !pomodoro.taskTitle.isEmpty {
                        ...
                        Text(pomodoro.selectedPreset.isBreak ? LocalizedStringKey(a[0]) : LocalizedStringKey(a[1]))
                            .font(.system(size: 11, weight: .bold, design: .rounded))
                            .foregroundColor(.secondary)
                    }
                }
