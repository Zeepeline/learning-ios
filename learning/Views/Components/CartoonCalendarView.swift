...
struct CartoonCalendarView: View {
    ...
    dynamic var body: some View {
        VStack(spacing: HIGSpacing.sm) {
                    ...
                    if let dayNumber = cell.dayNumber, let cellDate = cell.date {
                        ...
                        Button {
                            ...
                            VStack(spacing: 2) {
                                ...
                                // Task Dot Indicator atau Today Dot
                            ...
                            .background(
                                isSelected
                                    ? Color.cartoonCoral
                                    : (isToday ? Color.cartoonYellow.opacity(0.25) : Color.clear)
                            )
                            ...
                            .overlay(
                                RoundedRectangle(cornerRadius: 10)
                                    .stroke(
                                        Color.black,
                                        lineWidth: isSelected ? 2.0 : (isToday ? 1.0 : 0)
                                    )
                            )
                            .shadow(
            ...
            // 4. Banner Tanggal Terpilih (Memberikan Konfirmasi Visual Jelas)
            HStack(spacing: 6) {
                ...
                Text("Terpilih: ")
                    ...
                    .foregroundColor(.secondary)
                +
                Text(selectedDateSummaryString)
            ...
            // 5. Baris Pengaturan Waktu (Jika Diaktifkan)
            if showTimePicker {
                Divider()
    ...
    private var selectedDateSummaryString: String {
        let isToday = calendar.isDateInToday(selectedDate)
        let isTomorrow = calendar.isDateInTomorrow(selectedDate)
        let isYesterday = calendar.isDateInYesterday(selectedDate)
