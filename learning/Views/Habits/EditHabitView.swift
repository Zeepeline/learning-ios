...
// EditHabitView: helper methods kept here — its body reads private member(s): availableColors, availableIcons, categories, dismiss, errorMessage, frequencies, isFormValid, saveChanges, selectedCategory, selectedColorHex, selectedFrequency, selectedIcon, title.
...
extension EditHabitView {
    ...
    @MainActor func __patchSlots() -> [String: ([String]) -> AnyView] {
        ...
        __s["op_1ba94ca8fad67dec"] = { (a: [String]) in a.count >= 12 ? AnyView(VStack(spacing: HIGSpacing.md) {
                    // 1. Preview Kartun Interaktif
                    VStack(spacing: 8) {
        ...
        __s["op_439f7146655c73b5"] = { (a: [String]) in a.count >= 1 ? AnyView(Button(LocalizedStringKey(a[0])) {
                        saveChanges()
                    }
                    .font(.system(.body, design: .rounded).weight(.black))
                    .foregroundColor(isFormValid ? .black : .gray.opacity(0.5))
    ...
    @MainActor func __patchActionSlots() -> [String: () -> Void] {
        ...
        __a["actb46a3ff888d8645f"] = { saveChanges() }
        return __a
    }
    ...
    @MainActor func __patchEffectSlots() -> [String: (AnyView) -> AnyView] {
        ...
        __e["eff83e015274a922ab2"] = { (content: AnyView) -> AnyView in AnyView(content.onAppear {
            title = habit.title
            selectedCategory = habit.category
            selectedIcon = habit.icon
            selectedColorHex = habit.colorHex
